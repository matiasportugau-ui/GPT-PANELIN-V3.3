...
get_docker_compose_cmd() {
    ...
    if docker compose version >/dev/null 2>&1; then
        echo "docker compose"
    # Fall back to legacy v1 (docker-compose) if available
...
check_mcp_server() {
    ...
    if command -v docker >/dev/null 2>&1; then
        ...
        if [ -n "$compose_cmd" ]; then
            ...
            if $compose_cmd ps | grep -q "panelin-bot.*Up"; then
...
check_docker_resources() {
    ...
    if docker info > /dev/null 2>&1; then
        ...
        if [ "$IS_CI" = "true" ]; then
            log_warning "Docker daemon is not accessible (expected in CI)"
            return 0
        else
    ...
    if [ -n "$compose_cmd" ]; then
        ...
        if [ "$container_count" -gt 0 ]; then
            ...
            # In CI environments, no containers may be running - treat as warning
            if [ "$IS_CI" = "true" ]; then
                log_warning "No containers are currently running (expected in CI)"
            else
                log_warning "No containers are currently running"
            fi
        fi
    else
        log_warning "Docker Compose not available, skipping container count check"
    fi
