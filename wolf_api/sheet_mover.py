...
def _map_row(source: list, mapping: dict, target_cols: int) -> list:
    target = [""] * target_cols
...
async def mover_a_esperando(api_key: str = Depends(verify_api_key)):
    ...
    try:
        ...
        for idx, row, missing in reversed(rows_to_move):
            ...
            logger.info("Moved '%s' to Esperando (missing: %s)", cliente, missing)

        return MoveResult(action="mover_esperando", rows_moved=moved, details=details)

    except Exception as exc:
        logger.error("Error moviendo a Esperando: %s", exc, exc_info=True)
...
async def scan_admin(api_key: str = Depends(verify_api_key)):
    ...
    try:
        ...
        if enviados_count > 0:
            details.append(
                f"Nota: {enviados_count} filas con Estado=Enviado (trigger automático las mueve)"
            )
        ...
        for idx, row, missing in reversed(to_esperando):
            ...
            esp_row = _map_row(row, ADMIN_TO_ESPERANDO, ESPERANDO_COLS)
            esp_row[1] = f"Pendiente (falta: {', '.join(missing)})"
            esperando_ws.insert_row(
                esp_row, index=HEADER_ROWS + 1, value_input_option="USER_ENTERED"
            )
            admin_ws.delete_rows(idx + 1)
            moved += 1
        ...
        logger.error("Error en scan: %s", exc, exc_info=True)
        raise HTTPException(status_code=500, detail=f"Error: {exc}")
