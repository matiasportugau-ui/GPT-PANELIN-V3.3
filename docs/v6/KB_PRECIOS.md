# BMC URUGUAY — BASE DE PRECIOS Y COSTOS
## Última actualización: Enero 2026 | Moneda: USD

---

## 1. PANELES — Precio y Costo por m²

### ISODEC EPS (Techos y Cubiertas)
- Ancho útil: 1.12m | Largo máx: 14m | Pendiente mín: 7%
- Fijación: Varilla roscada 3/8" + tuercas

| Espesor | PVP/m² | Costo/m² | Autoportancia |
|---------|--------|----------|---------------|
| 100mm | 37.76 | 32.83 | 5.5m |
| 150mm | 42.48 | 36.94 | 7.5m |
| 200mm | 47.63 | 41.42 | 9.1m |
| 250mm | 52.36 | 45.53 | 10.4m |

Factor desperdicio: 1.12 (12%)

### ISODEC PIR (Techos y Cubiertas — mejor térmico)
- Ancho útil: 1.12m | Largo máx: 14m | Pendiente mín: 7%
- Fijación: Varilla roscada 3/8" + tuercas

| Espesor | PVP/m² | Costo/m² | Autoportancia |
|---------|--------|----------|---------------|
| 50mm | 41.83 | 36.37 | 3.5m |
| 80mm | 42.99 | 37.38 | 5.5m |
| 120mm | 51.38 | 44.68 | 7.6m |

Factor desperdicio: 1.12 (12%)

### ISOROOF 3G (Techos — trapezoidal)
- Ancho útil: 1.00m | Largo máx: 8.5m | Pendiente mín: 7%
- Fijación a madera: Caballete + Tornillo aguja (SIN varilla ni tuercas)
- Fijación a metal: Caballete + Tornillo punta mecha

| Espesor | PVP/m² | Costo/m² | Autoportancia |
|---------|--------|----------|---------------|
| 30mm | 39.95 | 34.74 | 2.8m |
| 40mm | 40.72 | 35.41 | 3.0m |
| 50mm | 44.00 | 38.26 | 3.3m |
| 80mm | 51.73 | 44.98 | 4.0m |

Factor desperdicio: 1.00 (sin desperdicio)

### ISOROOF Plus (Techos — premium)
- Ancho útil: 1.00m | Mismo sistema fijación que ISOROOF 3G

| Espesor | PVP/m² | Costo/m² | Autoportancia |
|---------|--------|----------|---------------|
| 80mm | 58.82 | 51.15 | 4.0m |

### ISOROOF Foil (Techos — económico)
- Ancho útil: 1.00m

| Espesor | PVP/m² | Costo/m² | Autoportancia |
|---------|--------|----------|---------------|
| 30mm | 32.41 | 28.18 | 2.8m |
| 50mm | 36.74 | 31.95 | 3.3m |

### ISOROOF Colonial
- Ancho útil: 1.00m | Largo máx: 8.5m
- Mismo sistema fijación que ISOROOF 3G
- **Nota:** Producto especial — cotizar solo con autorización del supervisor

| Espesor | PVP/m² | Costo/m² | Autoportancia |
|---------|--------|----------|---------------|
| 40mm | 62.07 | 53.97 | 3.0m (ref. ISOROOF 3G 40mm) |

### ISOPANEL EPS (Paredes y Fachadas)
- Ancho útil: 1.10m | Largo máx: 12m
- Fijación: Varilla roscada 3/8" + tuercas

| Espesor | PVP/m² | Costo/m² |
|---------|--------|----------|
| 50mm | 41.88 | — |
| 100mm | 46.00 | — |
| 150mm | 51.50 | — |
| 200mm | 57.00 | — |
| 250mm | 62.50 | — |

### ISOWALL PIR (Fachadas — premium)
- Ancho útil: 1.10m
- Fijación: Varilla roscada 3/8" + tuercas

| Espesor | PVP/m² | Costo/m² |
|---------|--------|----------|
| 50mm | 44.80 | 38.95 |
| 80mm | 53.42 | 46.45 |

### ISOFRIG SL (Cámaras frigoríficas)
| Espesor | PVP/m² | Costo/m² |
|---------|--------|----------|
| 40mm | 41.71 | 36.27 |
| 60mm | 47.40 | 41.22 |
| 80mm | 52.29 | 45.47 |
| 100mm | 58.01 | 50.44 |
| 120mm | 69.33 | 60.29 |
| 150mm | 70.38 | 61.20 |
| 180mm | 86.33 | 75.07 |

Factor desperdicio ISOFRIG: 1.10 (10%)
Factor desperdicio Cámara frigorífica paredes: 1.14 (14%)

---

## 2. ACCESORIOS — Precio y Costo por metro lineal (ML)

### Accesorios ISODEC

| Perfil | Largo barra | PVP/ml | Costo/ml |
|--------|-------------|--------|----------|
| Gotero Frontal 100mm | 3.03m | 5.22 | 4.31 |
| Gotero Lateral 100mm | 3.00m | 6.92 | 5.77 |
| Perfil Aluminio 5852 | 6.80m | 9.30 | 7.75 |
| Cumbrera ISODEC | 3.03m | 7.86 | 6.48 |
| Babeta Atornillar | 3.00m | 4.06 | 3.39 |
| Babeta Empotrar | 3.00m | 4.06 | 3.39 |
| Canalón ISODEC 100mm | 3.03m | 23.18 | 19.13 |
| Soporte Canalón ISODEC | 3.00m | 5.31 | 4.43 |

### Accesorios ISOROOF

| Perfil | Largo barra | PVP/ml | Costo/ml |
|--------|-------------|--------|----------|
| Gotero Frontal Simple | 3.03m | 6.60 | — |
| Gotero Frontal con Greca | 3.03m | — | — |
| Gotero Lateral ISOROOF | 3.00m | 9.48 | — |
| Cumbrera Roof 3G | 3.03m | 13.20 | — |
| Canalón Doble 80mm | 3.03m | 27.81 | — |
| Soporte de Canalón ISOROOF | 3.00m | 4.23 | — |
| Limahoya estándar | 3.00m | 7.40 | — |

NOTA: Accesorios ISOROOF con costo marcado "—" → consultar catálogo actualizado o usar estimación de margen 20% (PVP / 1.20).

---

## 3. FIJACIONES — Precio y Costo por unidad

| Ítem | Especificación | PVP/und | Costo/und |
|------|----------------|---------|-----------|
| Silicona Neutra | 300ml pomo | 7.11 | 2.42 |
| Silicona Neutra (uso cotización) | 400g pomo | 6.08 | 2.42 |
| Varilla Roscada 3/8" | 1 metro | 2.43 | 1.15 |
| Varilla Roscada 5/16" | 1 metro | 1.90 | 0.90 |
| Tuerca Galvanizada 3/8" | unidad | 0.15 | 0.03 |
| Tuerca Galvanizada 5/16" | unidad | 0.12 | 0.02 |
| Arandela Carrocero | unidad | 2.00 | 0.29 |
| Arandela Plana | unidad | 0.24 | 0.03 |
| Arandela Polipropileno | unidad | 1.60 | 0.60 |
| Remache POP | unidad | 0.06 | 0.025 |
| Membrana Auto-adhesiva | rollo 0.3×10m | 36.20 | 24.10 |
| Espuma PU Expansiva | 750ml | 10.40 | 5.63 |
| Taco Expansivo 3/8" | unidad | 0.53 | 0.33 |
| Caballete Roof | unidad | 0.63 | 0.30 |
| Tornillo Aguja / P. Mecha | unidad | 0.78 | 0.35 |
| Tornillo T1 ½ | unidad | 0.06 | 0.025 |

---

## 4. MÁRGENES DE REFERENCIA POR CATEGORÍA

| Categoría | Margen estándar | Factor |
|-----------|----------------|--------|
| Paneles EPS | 15% + ajuste | ×1.15 + 0.01 |
| Paneles PIR | 15% | ×1.15 |
| Paneles ISOROOF | 15% | ×1.15 |
| Perfiles / Accesorios | 20% | ×1.20 |
| Accesorios Puerta Frigorífica | 30% | ×1.30 |
| Tornillería | 50-200% | ×1.50 a ×3.00 |
| Silicona | ~200% | ×3.00 |

---

## 5. IVA Y TOTALES

- IVA Uruguay = 22%
- Subtotal = suma de líneas (paneles + accesorios + anclaje + traslado)
- Total Final = Subtotal + IVA
- El motor PDF calcula todo automáticamente a partir de precios unitarios × cantidades
